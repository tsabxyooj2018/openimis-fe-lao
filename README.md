# openIMIS frontend — Lao deployment

Modified [openIMIS](https://openimis.org) frontend assembly (release **26.04**)
for a Lao national health insurance deployment.

**This repository exists to satisfy AGPL v3 section 13** — it is the Corresponding
Source for the frontend running at <https://www.openimislaos.site>. See
[NOTICE.md](NOTICE.md) for what was changed and why.

## What is different from upstream

- Bilingual Lao/English login labels
- Lao (`lo`) locale, and Lao-capable fonts ahead of Rubik/Roboto
- Institutional blue palette and a restyled login screen
- Ministry emblem in place of the openIMIS logo, with the AGPL credit restored
- Two upstream build fixes without which this image cannot be built at all

## Building

```bash
docker build -t openimis-fe-lao:26.04 .
```

Pushes to `main` build and publish `ghcr.io/<owner>/openimis-fe-lao:26.04`
automatically.
